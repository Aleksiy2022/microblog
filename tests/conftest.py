...
from .overrides_dependencies import ovr_get_current_user_by_api_key, ovr_scoped_session_db
...
async def set_test_settings():
    test_dir_uploaded_images = os.path.join(
        test_dir, "test_api", "test_routers", "uploaded_test_images",
    )
...
async def create_test_db():
    async with test_db_helper.engine.begin() as conn_create:
        await conn_create.run_sync(Base.metadata.create_all)

    await create_test_data_bd()
    yield

    async with test_db_helper.engine.begin() as conn_drop:
