...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "result": True,
                "user": {"id": 1, "name": "Aleksiy", "followers": [], "following": []},
            },
        ...
        (
            "some_very_very_long_wrong_api_key",
            422,
            {
                "result": False,
...
@pytest.mark.asyncio(scope="session")
async def test_create_user_following_node(
    async_client, user_id, exr_status_code, exp_response_json,
):
...
@pytest.mark.asyncio(scope="session")
async def test_delete_user_following_node(
    async_client, user_id, exr_status_code, exp_response_json,
):
    headers = {"Api-Key": "test"}
