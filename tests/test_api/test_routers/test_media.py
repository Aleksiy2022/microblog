...
@pytest.mark.parametrize(
    ...
    [
        (
            os.path.join(path, "test_images", "light_image.jpg"),
            "light_image.jpg",
            {"result": True, "media_id": 1},
        ),  # test with correct data
        (
            os.path.join(path, "test_images", "heavy_image.png"),
            "heavy_image.jpg",
            {
                "result": False,
        ...
        (
            os.path.join(path, "test_images", "not_image.txt"),
            "not_image.txt",
            {
                "result": False,
...
async def test_load_media(async_client, image_path, image_name, exp_response):
    ...
    response = await async_client.post(
        "http://127.0.0.1:8000/api/medias/", files={"file": (image_name, image_io)},
    )
